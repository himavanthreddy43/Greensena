...
export default function FamiliesPage() {
    ...
    const handleSaveMemberDetails = async () => {
        ...
        try {
            await axios.put(`/api/family_members/${selectedMember.member_id}`, editMemberForm);
            fetchFamilies();
    ...
    const handleDeleteSelectedMember = async () => {
        ...
        try {
            await axios.delete(`/api/family_members/${selectedMember.member_id}`);
            fetchFamilies();
    ...
    const handleDeleteFace = async (faceId) => {
        ...
        try {
            await axios.delete(`/api/face/${faceId}`);
    ...
    const submitNewMember = async (familyId) => {
        ...
        try {
            ...
            const res = await axios.post(`/api/family/${familyId}/member`, payload);
    ...
    const handleDeleteMember = async (memberId, familyId) => {
        ...
        try {
            await axios.delete(`/api/family_members/${memberId}`);
            // Update local state
            setFamilies(families.map(f => {
                if (f.family_id === familyId) {
    ...
    const saveEdit = async () => {
        setIsSaving(true);
        try {
            await axios.put(`/api/families/${editingFamily}`, editForm);
